...
public:
  test() {}
  test(int i, int j) : _i(i) { cout << "test(int, itn)" << endl; }
...
template <typename Signature> class SignalTrivial;

template <typename RET, typename... ARGS> class SignalTrivial<RET(ARGS...)> {
public:
  ...
  void print() { cout << sizeof...(ARGS) << endl; }
