...
func Test_CraateSession(t *testing.T) {
	setup()
	if err := users[0].Create(); err != nil {
		t.Error(err, "Cannot create user.")
	}
...
func Test_checkValidSession(t *testing.T) {
	setup()
	if err := users[0].Create(); err != nil {
		t.Error(err, "Cannot create uesr.")
	}
	session, err := users[0].CreateSession()
	if err != nil {
		t.Error(err, "Cannot create session")
	...
	valid, err := s.Check()
	if err != nil {
		t.Error(err, "Cannot check sessin")
	}
	if valid != true {
...
func Test_checkInvalidSession(t *testing.T) {
	...
	valid, err := s.Check()
	if err == nil {
		t.Error(err, "Sessin is not valid but is validated")
	}
	if valid == true {
...
func Test_DeleteSession(t *testing.T) {
	...
	valid, err := s.Check()
	if err == nil {
		t.Error(err, "Session is valid even thought delted")
	}
	if valid == true {
